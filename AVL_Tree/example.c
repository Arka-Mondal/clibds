...
int main(void)
{
  ...
  clibds_avl_initialize(&new_tree, int, comp_data);

  for (i = 0; i < 9; i++)
  {
    if (clibds_avl_insert(&new_tree, data_arr + i) == false)
    {
  ...
  count = clibds_avl_getdata(&new_tree, order, POSTORDER);

  for (i = 0; i < count; i++)
    printf("%d ", order[i]);
  putchar('\n');

  clibds_avl_remove(&new_tree, &(int){14});
